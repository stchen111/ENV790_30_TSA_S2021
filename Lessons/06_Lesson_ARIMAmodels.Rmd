...
ts_electricity_price <- ts(electricity_price_processed[,2:(nvar+1)],
                           ...
                           frequency=12) #start with (year, month) (year(..$Month[1]), month(..$Month[1]))
...
# lowercase plot (acf, pacf) from lag 0 (1)
```
...
decompose_residential_price <- decompose(ts_electricity_price[,"Residential"],"additive") #only selecting the residential column 
...
deseasonal_residential_price <- seasadj(decompose_residential_price)  # take the object from decompose()
...
Acf(df_residential$Residential,lag.max=40,main="Residential") #seasonality removed
Acf(df_residential$NonSeasonalResidential,lag.max=40,main="Non Sesonal Residential") #still have slight decay (yt is still dependent on previous observation ) 
...
head(deseasonal_residential_price)
```
...
#Null: model has unit root, not stationary
...
#Difference the data at lag 1 -> calculates the difference between first and 1+n/lag th value
deseasonal_residential_price_diff <- diff(deseasonal_residential_price,differences=1)
# differences = order of the function (how many times to call it)
# lag = which value (nth value) to subtract the first value

#Add the new series to our data frame
df_residential_full <- 
  df_residential %>% 
  cbind(ResidentialDiff = c(NA,as.numeric(deseasonal_residential_price_diff))) %>% #add NA to have same number of rows
  na.omit(residentialDiff) #omit rows with NA 
...
ggplot(df_residential_full, aes(x=Month)) +
      geom_line(aes(x=Month,y=Residential),color="black") +
      geom_line(aes(x=Month,y=NonSeasonalResidential),color="blue") +
      geom_line(aes(x=Month,y=ResidentialDiff),color="green")
