...
colnames(raw_inflow_data)=c("Month","Year", "HP1", "HP2","HP3","HP4", "HP5",
                            ...
                            "HP13", "HP14","HP15") #adding column name w colnames()=c()
...
inflow_seas_comp=array(0,nobs) #creating a vector of zeros - then replacing zeros with values
for(i in 1:nobs){
  inflow_seas_comp[i]=(beta_int+beta_coeff%*%dummies[i,])
} #replace vectors with inner prodcut
...
```{r}
#Mann Kendall Test
# SeasonalMannKendall(timeseries data[,specific column])
# MannKendall(timeseries data[,specific column])
...
#ADF test
# adf.test(seasonal data[,specific column], alternative="stationary")
