...
```{r}
#the function would be able to figure out the best replacing data
...
ggplot(full_wind_data, ) +
  geom_line(aes(x = Date, y = Wind_Clean), color = "red") + #the cleaned data
  geom_line(aes(x = Date, y = Wind), color = "black") + #original data
  ylab("Average Wind Speed") 
  
#(color="XXX") -> assign specific variable
...
wind_random <- decompose_wind_data$random #observed - seasonal - trend (remaninder basically)
...
# use prior and latter six observation to compute the seventh observation - use total of thirteen observations in total
...
for (i in 7:length(clean_wind_data)-6){
  rep_trend[i]=mean(clean_wind_data[i-6:i+6])
}
